...
with open('C:/personal_mirza/lomba_ndac/provinces_33_merged.geojson', encoding='utf-8') as f:
    geo33_str = f.read()

with open('C:/personal_mirza/lomba_ndac/provinces_33_knn.json', encoding='utf-8') as f:
    provs33_str = f.read()
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .method-callout b { color: var(--text); }
    ...
    /* Province Scope Toggle Switcher */
    .scope-switcher {
      display: flex;
      align-items: center;
      gap: 8px;
...
<body>
  ...
  <nav>
    <div class="nav-inner">
      ...
      <ul class="nav-links">
        ...
        <li><a href="#peta">Peta Spasial</a></li>
  ...
  <main class="container">
    ...
    <section class="hero" id="ringkasan">
      ...
      <p class="hero-desc">
        Pemodelan spasial-panel penggerak ekonomi terhadap Angka Harapan Hidup provinsi di Indonesia. Menyingkap paradoks pasar tenaga kerja subsisten, modal kesehatan Grossman, dan efek limpahan regional LeSage-Pace.
      </p>
      ...
      <!-- Metodologi Dual-Scope Explanation -->
      <div class="method-callout">
        <b>Pedoman Data (38 vs 33 Provinsi):</b> Di laporan dan PPT, seluruh Indonesia ditampilkan utuh (<b>38 Provinsi</b>) saat memotret fenomena paradoks nasional 2024 (Papua Pegunungan tingkat kerja tertinggi 98,68% tapi AHH terendah 64,76 tahun). Sementara pemodelan regresi spasial-panel (2013–2024, 396 observasi) dijalankan pada <b>33 Provinsi</b> seimbang karena 5 DOB baru belum memiliki rangkaian data historis lengkap. Anda dapat beralih antara kedua sudut pandang di bawah ini.
      </div>

      <!-- KPI Grid -->
      <div class="kpi-grid">
        <div class="kpi-card">
          <div class="kpi-label" id="kpi-gap-lbl">Gap Harapan Hidup 2024</div>
          <div class="kpi-val" style="color:var(--crimson);" id="kpi-gap-val">10,77 th</div>
          <div class="kpi-desc" id="kpi-gap-desc">Papua Pegunungan (64,76 th) vs DI Yogyakarta (75,53 th).</div>
        </div>
        <div class="kpi-card">
          <div class="kpi-label">Paradoks Ketenagakerjaan</div>
          <div class="kpi-val" style="color:var(--amber);" id="kpi-er-val">98,68%</div>
          <div class="kpi-desc" id="kpi-er-desc">Tingkat kerja tertinggi di Papua Peg., namun AHH justru terendah se-Indonesia.</div>
        </div>
        <div class="kpi-card">
    ...
    <section id="peta">
      <div class="section-head">
        <div class="section-eyebrow">Visualisasi Geografis Riil</div>
        <h2 class="section-title">Peta Spasial Indonesia & Matriks Bobot W (KNN k=4)</h2>
        <p class="section-subtitle">
          Peta poligon batas administrasi kepulauan Indonesia. Klik provinsi untuk menginspeksi data riil dan busur koneksi 4 tetangga terdekat.
        </p>
      </div>
        ...
        <div>
          <div class="map-toolbar">
            ...
            <div class="scope-switcher">
              <span class="scope-lbl">Cakupan:</span>
              <button class="scope-btn active" id="btn-scope-38" onclick="setScope(38)">38 Prov (Peta Lengkap)</button>
              <button class="scope-btn" id="btn-scope-33" onclick="setScope(33)">33 Prov (Panel SDM)</button>
            </div>
          </div>
    ...
    <section id="dekomposisi">
      ...
      <div class="calc-box">
        <div>
          <div style="font-size:14px;font-weight:700;margin-bottom:14px;">Kontrol Parameter SDM (N=396)</div>
  ...
  <script>
    ...
    let activeScope = 38; // Default 38 provinces so the full island map is shown
    let map;
    let geojsonLayer;
    ...
    function setScope(scopeNum) {
      activeScope = scopeNum;
      document.getElementById('btn-scope-38').classList.toggle('active', scopeNum === 38);
      document.getElementById('btn-scope-33').classList.toggle('active', scopeNum === 33);

      const kpiGapVal = document.getElementById('kpi-gap-val');
      const kpiGapDesc = document.getElementById('kpi-gap-desc');
      const kpiErVal = document.getElementById('kpi-er-val');
      const kpiErDesc = document.getElementById('kpi-er-desc');

      if (scopeNum === 38) {
        kpiGapVal.innerText = '10,77 th';
        kpiGapDesc.innerText = 'Papua Pegunungan (64,76 th) vs DI Yogyakarta (75,53 th).';
        kpiErVal.innerText = '98,68%';
        kpiErDesc.innerText = 'Tingkat kerja tertinggi di Papua Peg., namun AHH justru terendah se-Indonesia.';
        selectedProvinceName = 'Papua Pegunungan';
      } else {
        kpiGapVal.innerText = '9,21 th';
        kpiGapDesc.innerText = 'Sulawesi Barat (66,32 th) vs DI Yogyakarta (75,53 th).';
        kpiErVal.innerText = '98,21%';
        kpiErDesc.innerText = 'Tingkat kerja tertinggi panel di Bali (98,21%); AHH terendah di Sulbar (66,32 th).';
        selectedProvinceName = 'Sulawesi Barat';
      }
    ...
    function styleFeature(feature) {
      ...
      const isSelected = (p.canonical_name === selectedProvinceName || p.PROVINSI === selectedProvinceName);
    ...
    function drawGeoJson() {
      ...
      const currentGeo = DATASETS[activeScope].geo;
      ...
      geojsonLayer = L.geoJSON(currentGeo, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          layer.on({
            ...
            click: () => {
              selectProvinceByName(p.canonical_name || p.PROVINSI);
            }
          });

          layer.bindTooltip(`<b>${p.canonical_name || p.PROVINSI}</b><br>AHH: ${p.ahh} th · ER: ${p.er}%`, {
            sticky: true,
    ...
    function selectProvinceByName(name) {
      ...
      const stats = DATASETS[activeScope].stats;
      const stat = stats.find(s => s.name.toLowerCase() === name.toLowerCase() || name.toLowerCase().includes(s.name.toLowerCase()));
      if (!stat) return;
      ...
      } else if (stat.name === 'Sulawesi Barat' && activeScope === 33) {
        tagBox.innerHTML = '<span class="prov-tag tag-neg">AHH TERENDAH PANEL (66.32 th)</span>';
      ...
      const startCoord = [stat.lat, stat.lng];

      (stat.neighbors || []).forEach(nName => {
        const nStat = stats.find(s => s.name === nName);
        ...
        if (nStat) {
          const endCoord = [nStat.lat, nStat.lng];
      ...
      if (geojsonLayer) {
        geojsonLayer.eachLayer(layer => {
          ...
          const isSelected = (p.canonical_name === stat.name || p.PROVINSI === stat.name);
          layer.setStyle({
            weight: isSelected ? 3 : 1,
            color: isSelected ? '#0f766e' : '#ffffff',
            dashArray: isSelected ? '' : '2',
...
print('Updated index.html: default full 38 provinces with 33 panel toggle!')
