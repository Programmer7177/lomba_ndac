...
<body>
  ...
  <script>
      ...
      geojsonLayer = L.geoJSON(currentGeo, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          layer.on({
            ...
            click: () => {
              // EXACT match using canonical_name
    ...
    function selectProvinceByName(name) {
      ...
      // EXACT MATCHING to prevent "Maluku" vs "Maluku Utara" or "Papua" vs "Papua Pegunungan" collisions
      const stat = stats.find(s => s.name === name) || stats.find(s => s.name.toLowerCase() === name.toLowerCase());
      if (!stat) return;
      ...
      // Find the clicked polygon layer to get its exact geometric center
      ...
      if (geojsonLayer) {
        geojsonLayer.eachLayer(layer => {
          if (layer.feature && layer.feature.properties && layer.feature.properties.canonical_name === stat.name) {
            // Leaflet bounds center of this actual polygon
...
print('Updated index.html: EXACT canonical name matching & Leaflet getBounds() centroid anchor!')
