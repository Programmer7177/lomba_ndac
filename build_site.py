  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #leaflet-map {
      ...
      background-image: 
        linear-gradient(rgba(215, 210, 198, 0.35) 1px, transparent 1px),
        linear-gradient(90deg, rgba(215, 210, 198, 0.35) 1px, transparent 1px);
...
<body>
  ...
  <nav>
    <div class="nav-inner">
      ...
      <ul class="nav-links">
        ...
        <li><a href="#peta">Peta Spasial 38 Provinsi</a></li>
  ...
  <main class="container">
    ...
    <section class="hero" id="ringkasan">
      ...
      <p class="hero-desc">
        Pemodelan spasial-panel penggerak ekonomi terhadap Angka Harapan Hidup provinsi di Indonesia (2013–2024). Menyingkap paradoks pasar tenaga kerja subsisten, modal kesehatan, dan efek limpahan regional.
      </p>
      ...
      <div class="kpi-grid">
        ...
        <div class="kpi-card">
          <div class="kpi-label">Paradoks Papua Peg.</div>
          <div class="kpi-val" style="color:var(--amber);">98,68%</div>
          <div class="kpi-desc">Tingkat kerja tertinggi se-Indonesia, namun AHH justru terendah nasional.</div>
        </div>
        <div class="kpi-card">
    ...
    <section id="peta">
      <div class="section-head">
        ...
        <p class="section-subtitle">
          Peta poligon batas administrasi 38 provinsi Indonesia. Klik provinsi untuk menginspeksi data riil dan busur koneksi 4 tetangga terdekat.
        </p>
      </div>
        ...
        <div>
          <div class="map-toolbar">
            ...
            <span style="font-size:11px;font-family:var(--font-mono);color:var(--text-subtle);">
              Peta Poligon 38 Provinsi · Leaflet
            </span>
          </div>
  ...
  <script>
    ...
    function initMap() {
      ...
      const indoBounds = L.latLngBounds([[-12.0, 93.0], [8.0, 143.0]]);
      ...
      // Fit map bounds to Indonesian territory cleanly
      if (geojsonLayer) {
        map.fitBounds(geojsonLayer.getBounds(), { padding: [20, 20], maxZoom: 6 });
      } else {
        map.fitBounds(indoBounds, { padding: [20, 20] });
      }

      selectProvinceByName(selectedProvinceName, false);
    }

    function getColor(val, metric) {
      if (metric === 'ahh') {
        // Range approx 64 to 76
      ...
      geojsonLayer = L.geoJSON(GEOJSON_DATA, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          layer.on({
            mouseover: (e) => {
              const l = e.target;
              l.setStyle({ weight: 2.5, color: '#0f766e', fillOpacity: 0.88 });
              l.bringToFront();
            },
            mouseout: (e) => {
            ...
            click: () => {
              selectProvinceByName(p.canonical_name || p.PROVINSI, true);
            }
          });

          let valDisplay = '';
          if (currentMetric === 'ahh') valDisplay = `AHH: ${p.ahh.toFixed(2)} th`;
          else if (currentMetric === 'er') valDisplay = `Tingkat Kerja: ${p.er.toFixed(2)}%`;
          else if (currentMetric === 'gdp') valDisplay = `PDRB/kap: Rp ${(p.gdp_percap / 1000).toFixed(1)}M`;
          else if (currentMetric === 'resid') valDisplay = `Residual: ${(p.resid >= 0 ? '+' : '') + p.resid.toFixed(2)} th`;

          layer.bindTooltip(`<b>${p.canonical_name || p.PROVINSI}</b><br><span style="font-size:11px;color:#0f766e;font-weight:600;">${valDisplay}</span>`, {
            sticky: true,
    ...
    function selectProvinceByName(name, shouldPan = true) {
      ...
      (stat.neighbors || []).forEach(nName => {
        const nStat = PROVINCES_STAT.find(s => s.name.toLowerCase() === nName.toLowerCase() || nName.toLowerCase().includes(s.name.toLowerCase()));
        const item = document.createElement('div');
        item.className = 'knn-item';
        item.innerHTML = `<span>${nName}</span><b style="color:var(--teal);">${nStat ? nStat.ahh : '-'} th</b>`;
        item.onclick = () => selectProvinceByName(nName, true);
      ...
      if (geojsonLayer) {
        geojsonLayer.eachLayer(layer => {
          ...
          if (isSelected && shouldPan) {
            map.flyTo([stat.lat, stat.lng], Math.max(map.getZoom(), 5.5), { duration: 0.8 });
          }
        });
      }
    }
...
print('Successfully generated C:/personal_mirza/lomba_ndac/index.html')
