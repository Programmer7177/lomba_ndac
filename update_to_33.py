...
<head>
  ...
  <title>Kuantitas Tanpa Kualitas — Riset NDAC EPSILON 2026 (33 Provinsi Panel Seimbang)</title>
...
<body>
  ...
  <nav>
    <div class="nav-inner">
      ...
      <ul class="nav-links">
        ...
        <li><a href="#peta">Peta 33 Provinsi Panel</a></li>
  ...
  <main class="container">
    ...
    <section class="hero" id="ringkasan">
      <div class="hero-tag">
        <span></span> NATIONAL DATA ANALYSIS COMPETITION (NDAC) 2026 · PANEL SEIMBANG (N=396)
      </div>
      <h1 class="hero-title">Kuantitas Tanpa Kualitas</h1>
      <p class="hero-desc">
        Pemodelan spasial-panel penggerak ekonomi terhadap Angka Harapan Hidup pada <b>33 provinsi di Indonesia</b> (2013–2024). Menyingkap paradoks pasar tenaga kerja subsisten, depresiasi modal kesehatan Grossman, dan efek limpahan regional LeSage-Pace.
      </p>
      ...
      <div class="kpi-grid">
        <div class="kpi-card">
          <div class="kpi-label">Panel Seimbang</div>
          <div class="kpi-val" style="color:var(--teal);">33 Prov</div>
          <div class="kpi-desc">12 tahun (2013–2024), 396 observasi penuh bebas data hilang.</div>
        </div>
        <div class="kpi-card">
          <div class="kpi-label">Gap Harapan Hidup Panel</div>
          <div class="kpi-val" style="color:var(--crimson);">9,21 th</div>
          <div class="kpi-desc">Sulawesi Barat (66,32 th) vs DI Yogyakarta (75,53 th) tahun 2024.</div>
        </div>
        <div class="kpi-card">
    ...
    <section id="peta">
      <div class="section-head">
        <div class="section-eyebrow">Visualisasi Geografis 33 Provinsi Panel</div>
        <h2 class="section-title">Peta Spasial Indonesia & Matriks Bobot W (KNN k=4)</h2>
        <p class="section-subtitle">
          Peta poligon batas administrasi 33 provinsi unit pemodelan. Klik provinsi untuk menginspeksi data riil 2024 dan busur koneksi 4 tetangga terdekat.
        </p>
      </div>
      ...
      <div class="map-container">
        <div>
          <div class="map-toolbar">
            ...
            <span style="font-size:11px;font-family:var(--font-mono);color:var(--text-subtle);">
              Peta Poligon Murni 33 Provinsi · Tanpa Tile Asing
            </span>
          </div>
        ...
        <div class="prov-panel">
          <div class="prov-card">
            <div class="prov-name" id="p-name">Sulawesi Barat</div>
            <div id="p-tag"><span class="prov-tag tag-neg">AHH TERENDAH PANEL</span></div>

            <div class="prov-stat-grid">
              <div class="prov-stat-box">
                <div class="prov-stat-lbl">Harapan Hidup (AHH)</div>
                <div class="prov-stat-val" id="p-ahh" style="color:var(--crimson);">66.32 th</div>
              </div>
              <div class="prov-stat-box">
                <div class="prov-stat-lbl">Tingkat Kerja (ER)</div>
                <div class="prov-stat-val" id="p-er" style="color:var(--teal);">97.32%</div>
              </div>
              <div class="prov-stat-box">
                <div class="prov-stat-lbl">PDRB per Kapita</div>
                <div class="prov-stat-val" id="p-gdp">Rp 42.1M</div>
              </div>
              <div class="prov-stat-box">
                <div class="prov-stat-lbl">Residual Model</div>
                <div class="prov-stat-val" id="p-resid">-3.24 th</div>
              </div>
            </div>
    ...
    <section id="dekomposisi">
      <div class="section-head">
        ...
        <p class="section-subtitle">
          Uji langsung amplifikasi pengganda spasial dan transmisi efek limpahan pada Spatial Durbin Model.
        </p>
      </div>
  ...
  <script>
    ...
    const PROVINCES_STAT = ''' + provs_str + ''';

    let map;
    let geojsonLayer;
    let neighborLinesLayer;
    let currentMetric = 'ahh';
    let selectedProvinceName = 'Sulawesi Barat';
    ...
    function initMap() {
      ...
      // Auto fit to Indonesian territory
      if (geojsonLayer) {
    ...
    function selectProvinceByName(name) {
      ...
      const stat = PROVINCES_STAT.find(s => s.name.toLowerCase() === name.toLowerCase() || name.toLowerCase().includes(s.name.toLowerCase()));
      if (!stat) return;
...
print('Updated index.html to exactly 33 provinces!')
